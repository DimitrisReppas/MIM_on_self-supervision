...
from util.masking.choose_mask import *
from util.masking.contrastive_mask_for_pair_selection import *
from util.loader import ImageFolderMask, ImageFolderMask2
from util.models.head import iBOTHead
...
def get_args_parser():
    ...
    parser.add_argument('--lambda3', default=1.0, type=float, help="""loss weight for contrastive
        loss over Teacher's patch tokens (Default: 1.0)""")
    ...
    # Masking parameters
    parser.add_argument("--mask", default="raw", type=str, help="The type of masking can be one of the following: raw, log_raw, power, rollout_all_, rollout_last, rollout_mid_")
    parser.add_argument("--layer_mask", default=11, type=int, help="This is the layer from which we generate the mask. It can be an integer from 0 to 11 in order to choose one of the 12 layers. For choosing one of the 3 rollout masking methods this argument is not taken into account.")
    parser.add_argument("--power", default=0.5, type=float, help="This is a parameter needed when using the power --mask. If you use different kind of masking this argument is not taken into account.The smaller the value the more unsalient regions you take into account. Give values < 1.0")
    # Data loading options
    parser.add_argument("--data_loading", default="subset", type=str, help="If we load the whole Imagenet or not. There are only two options: subset or full ")
    # Contrastive parameters
    parser.add_argument('--cont_dense_temp', type=float, default=0.2, help="""The temperature t for the dense contrastive loss.""")
    parser.add_argument("--k_anchors", default=3, type=int, help="Number of anchors used per image for calculating the contrastive loss.")
    return parser

def train_ibot(args):
    # utils.init_distributed_mode(args) # init distributed mode from the source code https://github.com/bytedance/ibot
    local_rank = utils.init_distributed_mode_jean_jay() # init distributed mode when using Jean Jay clusters
    ...
    if args.data_loading == 'subset': # when using the subset option, the first 20% samples of each Imagenet class are loaded
    ...
    elif args.data_loading == 'full': # when using the full option, the whole Imagenet is loaded
        dataset = ImageFolderMask2(
            args.data_path, 
            transform=transform,
            patch_size=pred_size,
            pred_ratio=args.pred_ratio,
        ...
        print("You gave a false argument")
...
def train_one_epoch(student, teacher, teacher_without_ddp, ibot_loss, data_loader,
    ...
    for it, (images, labels, masks) in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            ...
            attn_m = teacher_output[2]  # receive the attentions of the global crops for all layers
            teacher_output = teacher_output[:2]
            # Create the mask from your preference for each global crop
            masks = masking_mechanism(attn_m, args.batch_size_per_gpu, args.global_crops_number, args.mask, args.layer_mask, args.power, high=True)  
            # Find positive and negative pairs between the glob crops for calculating an extra dense contrastive loss (3rd of the main loss)
            khigh, klow = positive_negative_selection(attn_m, args.k_anchors, args.batch_size_per_gpu, args.global_crops_number, args.mask, args.layer_mask, args.power, high=True)

            student_output = student(images[:args.global_crops_number], mask=masks[:args.global_crops_number])
            student_output = student_output[:2] 
            # get local views
            ...
            all_loss = ibot_loss(student_output, teacher_output, student_local_cls, masks, khigh, klow, args.k_anchors, epoch, args.cont_dense_temp)
...
class iBOTLoss(nn.Module):
    ...
    def forward(self, student_output, teacher_output, student_local_cls, student_mask, khigh, klow, k_anchors, epoch, cont_dense_temp):
        """
        ...
        # Computation of 3rd loss term (dense contrastive loss on teacher's embeddings)                                                               
        pos = torch.zeros(teacher_patch_c[0].shape[0], k_anchors*2, teacher_patch_c[0].shape[2]).cuda()
        neg = torch.zeros(teacher_patch_c[0].shape[0], k_anchors*2, teacher_patch_c[0].shape[2]).cuda()

        tea1 = teacher_patch_c[0]
        tea2 = teacher_patch_c[1]

        khigh = khigh.type(torch.long).cuda()
        klow = klow.type(torch.long).cuda()
        
        
        for t in range(len(teacher_patch_c[0])):  # Positives and negatives selection
            pos[t,0:k_anchors,:] = torch.index_select(tea1[t,:,:], 0, khigh[t,:])
            pos[t,k_anchors:2*k_anchors,:] = torch.index_select(tea2[t,:,:], 0, khigh[t+len(teacher_patch_c[0]),:])
            neg[t,0:k_anchors,:] = torch.index_select(tea1[t,:,:], 0, klow[t,:])
            neg[t,k_anchors:2*k_anchors,:] = torch.index_select(tea2[t,:,:], 0, klow[t+len(teacher_patch_c[0]),:])

        pos_l2 = F.normalize(pos, p=2, dim=2) # L2 normalization of positives
        neg_l2 = F.normalize(neg, p=2, dim=2) # L2 normalization of negatives
        
        ppos_sim = torch.bmm(pos_l2, pos_l2.permute(0, 2, 1))
        pneg_sim_texp = (torch.exp(torch.bmm(pos_l2, neg_l2.permute(0, 2, 1)) / cont_dense_temp)).cuda()
        ppos_sim_id = ppos_sim - torch.eye(2*k_anchors).cuda()
        fullterm1 = -1 * torch.sum(torch.sum(torch.sum(ppos_sim_id))) / cont_dense_temp
        fullterm2 = torch.zeros(len(teacher_patch_c[0])).cuda()
        for t in range(2*k_anchors):
            allNegSim = torch.squeeze (torch.sum(pneg_sim_texp[:,t,:],1) )
            for l in ([*range(0, t)] + [*range(t+1, 2*k_anchors)]):
                fullterm2 = fullterm2 + torch.squeeze (torch.log( ppos_sim_id[:,t,l] + allNegSim ))
            fullterm2f = torch.sum(fullterm2)
        
        # Computation of main iBOT loss
        ...
        contrastive = (fullterm1+fullterm2f) / (2 * k_anchors * len(teacher_patch_c[0])) * self.lambda3 # Dense contrastive loss as a 3rd term
        total_loss = dict(cls=total_loss1, patch=total_loss2, contrastive=contrastive, loss=total_loss1 + total_loss2 + contrastive)
...
if __name__ == '__main__':
    ...
    mask_list = ["raw", "log_raw", "power", "rollout_all_", "rollout_last", "rollout_mid_"]
    if args.mask not in mask_list:
                raise Exception(f"mask should be one of {mask_list}")
