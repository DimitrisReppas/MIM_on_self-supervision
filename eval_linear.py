...
import util.models_evaluation as models_evaluation
from util.loader import ImageFolder, Listed_ImageFolder
...
def eval_linear(args):
    ...
    if args.data_loading == 'subset': # when using the subset option, the first 20% samples of each Imagenet class are loaded
        ...
        dataset_train = Listed_ImageFolder(traindir, annotations_file=path_20_imagenet, transform=train_transform)
    ...
    if 'swin' in args.arch:
        args.patch_size = 4
        model = models_evaluation.__dict__[args.arch](
    ...
    linear_classifier = nn.parallel.DistributedDataParallel(linear_classifier, device_ids=[local_rank], output_device=local_rank, broadcast_buffers=False)
