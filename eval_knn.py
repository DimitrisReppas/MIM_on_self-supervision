...
from util.loader import ImageFolderInstance, ImageFolderInstance2
...
def extract_feature_pipeline(args):
    ...
    if args.data_loading == 'subset': # when using the subset option, the first 20% samples of each Imagenet class are loaded
        ...
        dataset_train = ImageFolderInstance(traindir, annotations_file=path_20_imagenet, transform=transform)
    elif args.data_loading == 'full': # when using the full option, the whole Imagenet is loaded
        dataset_train = ImageFolderInstance2(traindir, transform=transform)
    else:
        print("You gave a false argument")
    dataset_val = ImageFolderInstance2(valdir, transform=transform)
