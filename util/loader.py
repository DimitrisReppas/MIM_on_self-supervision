...
# classes used for loading the 20% of Imagenet(the samples are given through a specific .txt file)
...
# classes used for loading the whole Imagenet

class ImageFolderInstance2(ImageFolder):
    def __getitem__(self, index):
        img, target = super(ImageFolderInstance2, self).__getitem__(index)
        return img, target, index

class ImageFolderMask2(ImageFolder):
    def __init__(self, *args, patch_size, pred_ratio, pred_ratio_var, pred_aspect_ratio, 
                 pred_shape='block', pred_start_epoch=0, **kwargs):
        super(ImageFolderMask2, self).__init__(*args, **kwargs)
        self.psz = patch_size
        self.pred_ratio = pred_ratio[0] if isinstance(pred_ratio, list) and \
            len(pred_ratio) == 1 else pred_ratio
        self.pred_ratio_var = pred_ratio_var[0] if isinstance(pred_ratio_var, list) and \
    ...
    def __getitem__(self, index):
        output = super(ImageFolderMask2, self).__getitem__(index)
