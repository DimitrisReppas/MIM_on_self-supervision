...
def masking_mechanism(attentions, batch_per_gpu, global_crops, method, layer=11, power=0.5, high=True):  # function for creating masks (based on attention) for each crop.
    attn_m = attentions
    ...
    if method[:3] == 'raw':
        ...
        masks = create_mask(aver_of_all_heads, batch_per_gpu, global_crops, high) # create the mask
    elif method[:3] == 'log':
        attn_cls_all_heads=attn_m[layer][:,:,0,1:]
        aver_of_all_heads=avg_log(attn_cls_all_heads)
        masks = create_mask(aver_of_all_heads, batch_per_gpu, global_crops, high)
    elif method[:3] == 'pow':
        attn_cls_all_heads=attn_m[layer][:,:,0,1:]
        ...
        if method[-4:] == "all_": # take into account all layer
            ...
            multip = avg_plus_I[8] @ avg_plus_I[9] @ avg_plus_I[10] @ avg_plus_I[11]
        ...
        masks = create_mask(cls_A, batch_per_gpu, global_crops, high) # create the mask
    return(masks)
...
def create_mask(aver_of_all_heads, batch_per_gpu, global_crops, high):
    ...
    for i in range(0, global_crops):
        length = length + batch
        mask = reshaped_cls[index:length]
        masks.append(mask)
        index = index + batch
