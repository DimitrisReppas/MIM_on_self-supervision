...
from util.masking.multi_choose_mask import *
...
def get_args_parser():
    ...
    # Masking parameters
    parser.add_argument("--mask", default=["raw", "raw"] , type=str, nargs='+', help="It is a list of the kind of masking you will use on each global crop.The number of elements of the list must be equal to the number of global crops.The type of masking can be one of the following: raw, log_raw, power, rollout_all_, rollout_last, rollout_mid_")
    parser.add_argument("--layer_mask", default=[11, 4], type=int, nargs='+', help="This is a list consist of the layer from which we generate the mask. It can be an integer from 0 to 11 in order to choose one of the 12 layers. When choosing one of the 3 rollout masking methods this argument is not taken into account.The number of elements of the list must be equal to the number of global crops.")
    parser.add_argument("--power", default=[0.5, 0.5], type=float, nargs='+', help="This is a list consist of parameters needed if using the power --mask. If you use different kind of masking this argument is not taken into account.The number of elements of the list must be equal to the number of global crops.")
...
def train_one_epoch(student, teacher, teacher_without_ddp, ibot_loss, data_loader,
    ...
    for it, (images, labels, masks) in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        with torch.cuda.amp.autocast(fp16_scaler is not None):
            ...
            attn_m = torch.stack(attn_m, dim=0) 
            index, length = 0, 0
            masks = []
            # Create the mask from your preference for each global crop
            for i in range(0, args.global_crops_number):
                length = length + args.batch_size_per_gpu
                attn = attn_m[:,index:length,:,:,:]
                mask = masking_mechanism(attn, args.mask[i], args.layer_mask[i], args.power[i], high=True)
                masks.append(mask)
                index = index + args.batch_size_per_gpu
...
if __name__ == '__main__':
    ...
    if len(args.mask) != args.global_crops_number or len(args.layer_mask) != args.global_crops_number or len(args.power) != args.global_crops_number :
                raise Exception(f"--mask, --layer_mask, --power arguments must have the same number as --global_crops_number")
